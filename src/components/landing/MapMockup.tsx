...
function HeatmapCell({ color, index }: HeatmapCellProps) {
  ...
  const rank = index < 5 ? index + 1 : index < 10 ? index - 3 : index < 15 ? index - 8 : index < 20 ? index - 12 : index - 17;
  ...
  return (
    <motion.div
      ...
      transition={isCenter ? {
        ...
        default: { delay: 0.5 + index * 0.01, type: 'spring', stiffness: 150, damping: 15 }
      } : {
      ...
      className={cn(
        "relative flex h-8 w-8 items-center justify-center rounded-lg text-[10px] font-bold border border-black/10 sm:h-12 sm:w-12 sm:text-xs transition-colors cursor-pointer",
