...
function CompetitorRow({ 
  ...
  const medalStyles = [
    'bg-amber-500 text-white shadow-sm shadow-amber-500/50', // 1st
    'bg-slate-400 text-white',                             // 2nd
    'bg-amber-700 text-white',                             // 3rd
  ];

  return (
    <tr className={`transition-all hover:bg-primary/5 ${isTarget ? 'bg-primary/10' : ''}`}>
      <td className="p-4 align-middle">
        <div className={`flex h-7 w-7 items-center justify-center rounded-lg text-xs font-bold ${
          idx < 3 ? medalStyles[idx] : 'bg-secondary text-muted-foreground'
        }`}>
          {idx + 1}
        </div>
      </td>
      ...
      <div className="flex flex-col sm:flex-row gap-5 items-center">
        ...
        <div className="flex-1 text-center sm:text-left space-y-1">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            El líder del mercado local es <strong className="text-foreground">{cleanBusinessName(leaderName)}</strong>, quien captura el <strong className="text-primary">{marketShare}%</strong> de las apariciones en el Top 3 (Local Pack).
          </p>
        </div>
